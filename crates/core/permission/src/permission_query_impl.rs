...
pub async fn calculate_server_permissions<P: PermissionQuery>(query: &mut P) -> PermissionValue {
  if query.is_privileged().await || query.is_server_owner().await {
    return ChannelPermission::GrantAllSafe.into();
  }

  if !query.is_server_member().await {
    return 0_u64.into();
  }
...
pub async fn calculate_channel_permissions<P: PermissionQuery>(query: &mut P) -> PermissionValue {
  if query.is_privileged().await {
    return ChannelPermission::GrantAllSafe.into();
  }

  match query.get_channel_type().await {
    ChannelType::SavedMessages => {
      if query.is_channel_owner().await {
    ...
    ChannelType::DirectMessage => {
      if query.is_part_of_the_channel().await {
    ...
    ChannelType::Group => {
      ...
      } else if query.is_part_of_the_channel().await {
        (*DEFAULT_PERMISSION_VIEW_ONLY | query.get_default_channel_permissions().await.allow).into()
      } else {
        0_u64.into()
      }
    }
    ChannelType::ServerChannel => {
      ...
      if query.is_channel_owner().await {
        ChannelPermission::GrantAllSafe.into()
      } else if query.is_server_member().await {
...
pub async fn calculate_user_permissions<P: PermissionQuery>(query: &mut P) -> PermissionValue {
  ...
  if query.have_mutual_connection().await {
    ...
    if query.user_is_bot().await | query.is_bot().await {
